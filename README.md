...
## Documentation [Daily Development Log (Notion)]
- https://useful-tarsier-ec6.notion.site/DE-Project-3c09cd4b60ef8086ad22d3ae0fe7d9a8?source=copy_link
...
단순히 데이터를 적재하고 시각화하는 것에 그치지 않고 다음과 같은 질문을 중심으로 설계하였다. (생각중)
...
- 장애 발생 시 데이터를 다시 처리할 수 있는 구조인가? (읽기속도, 쓰기속도의 느림 현상을 방지하기 위한 것?)
- 파이프라인 실패 알람 구조는 어떻게 되는가? (airflow - 에러메세지는 어떻게 구성할것인지/ 에러로그 분석 가능한 에이전트)
